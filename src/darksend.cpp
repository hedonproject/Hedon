...
{
    if(fLiteMode) return; // ignore all Hedon related functionality
...
/*
    ...
    allow endless transaction that would bloat Hedon and make it unusable. To
...
bool CDarkSendSigner::IsVinAssociatedWithPubkey(const CTxIn& txin, const CPubKey& pubkey)
{
    ...
    if(GetTransaction(txin.prevout.hash, tx, Params().GetConsensus(), hash, true)) {
        BOOST_FOREACH(CTxOut out, tx.vout)
            if(out.nValue == 200000*COIN && out.scriptPubKey == payee) return true;
    }
...
{
    if(fLiteMode) return; // disable all Hedon specific functionality
    ...
    RenameThread("hedon-privatesend");
