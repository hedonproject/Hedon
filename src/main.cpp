...
#if defined(NDEBUG)
# error "Hedon cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "HedonCoin Signed Message:\n";
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
	...
	if (!nPrevHeight && nPrevHeight == 0) {
    	...
    	} else if (nPrevHeight < 300) {
        	return 10 * COIN;
    	} else if (nPrevHeight >= 300 && nPrevHeight < 1000) {
        	return 250 * COIN;
...
{
	if (nHeight >= 3000 && nHeight < 4000) {
		return blockValue * 0.30;
	}else if (nHeight >= 4000 && nHeight < 5000) {
		return blockValue * 0.40;
	}else if (nHeight >= 5000 && nHeight < 6000) {
		return blockValue * 0.50;
	}else if (nHeight >= 6000 && nHeight < 9000) {
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // HDN : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(HDN): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(HDN): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // HDN : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(HDN): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(HDN): spork is off, skipping transaction locking checks\n");
    }
...
bool static AlreadyHave(const CInv& inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    /*
        Hedon Related Inventory Messages
