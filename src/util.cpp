...
const char * const BITCOIN_CONF_FILENAME = "hedon.conf";
const char * const BITCOIN_PID_FILENAME = "hedond.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "hedon" is a composite category enabling all Hedon-related debug output
            if(ptrCategory->count(string("hedon"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\HedonCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\HedonCore
    // Mac: ~/Library/Application Support/HedonCore
    // Unix: ~/.hedoncore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "HedonCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/HedonCore";
#else
    // Unix
    return pathRet / ".hedoncore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty hedon.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override hedon.conf
